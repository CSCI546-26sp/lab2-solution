...
inline void Raft::start_server() {
  ...
  builder.RegisterService(static_cast<raftpb::RaftService::Service*>(this)); /* replace nullptr with actual gRPC service */

  std::unique_ptr<Server> server(builder.BuildAndStart());
  logger->info("Raft server {} listening on {}", id_, listening_addr);
...
Raft::create_context(uint64_t to) const {
  ...
  context->AddMetadata("from", std::to_string(this->id_));
